...
1. `live-assistant wait --timeout-sec <秒>` でイベントを待機する。タイムアウト秒数は状況に応じて判断。`new` があれば返答を作り `live-assistant speak "<返答>"` する。`speak` はデフォルトで再生完了まで待つ。
2. 自律行動（後述）が必要か判断し、必要なら実行する。
3. ループ先頭に戻る。

## Response Before Action

時間のかかる操作を行う際は、`speak` で一言伝えてから実行する。

## Priority Rules
`source: "mic"` を最優先で処理する。
...
- **古くなった情報・不要になった情報は積極的に削除する。** ゲーム情報、一時的な話題、状況が変わった記録は見つけ次第消す。
